...
app.use('/api/study-records', studyRecordsRouter)

app.use('/api/study-recommendations', studyRecommendationsRouter)
...
app.use('/api/review-quest-drafts', reviewQuestDraftsRouter)

app.use('/api/review-quests', reviewQuestsRouter)
...
app.use((error: unknown, _request: Request, response: Response, _next: NextFunction) => {
  ...
  if (
    ...
    error.message === 'JPG 또는 PNG 이미지만 업로드할 수 있습니다.'
  ) {
    response.status(400).json({
      success: false,
