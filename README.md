...
- 주간 학습 대시보드와 날짜별 하루 목표 관리
- 학습 기록·완료 순공 세션 기반 현재/최고 연속 학습 기록
- 학습 기록 작성, 목록, 상세보기, 삭제
...
- 집중·실습 모드 공부 타이머와 일시정지·재개·종료
- 타이머 완료 횟수, 평균 집중 시간, 목표 달성률, 중단 횟수 통계
- PostgreSQL 기반 영구 저장
- JWT 쿠키 기반 로그인과 개인정보 수정
...
6. 기존 데이터베이스에 공부 타이머 기능 추가: `backend/database/study_sessions.sql`
7. 기존 데이터베이스에 날짜별 하루 목표 기능 추가: `backend/database/daily_goals.sql`

새 데이터베이스에는 `schema.sql`에 복습 문제, AI 학습 추천, 학습 기록 유형 컬럼, 공부 타이머, 하루 목표 테이블까지 포함되어 있으므로 3번부터 7번까지의 마이그레이션을 다시 실행할 필요가 없습니다.
...
회원가입·로그인 후 발급되는 HTTP 전용 JWT 쿠키로 사용자별 데이터 접근을 인증합니다. 하루 목표, 학습 기록, 순공 세션, 개인정보 API는 로그인한 사용자 범위에서만 동작합니다.
